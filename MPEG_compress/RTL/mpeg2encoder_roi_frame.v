...
module mpeg2encoder #(
    ...
    parameter  Q_LEVEL      = 4,    // quantize level, must be 1, 2, 3 or 4. The larger the Q_LEVEL, the higher compression ratio and the lower quality.
    // ROI-based quantization parameters
    // Q_LEVEL_ROI : Q-level used for ROI macroblocks (bigger => stronger quantization, more compression)
    // Q_LEVEL_BG  : Q-level used for non-ROI macroblocks (we default to original Q_LEVEL)
    parameter  Q_LEVEL_ROI  = 2,   // strong quantization in ROI for visible effect (tune as needed)
    parameter  Q_LEVEL_BG   = Q_LEVEL,
    // ROI window defined in macroblock coordinates (x16,y16)
    // Here: top row, leftmost 4 macroblocks: x16 = 0..3, y16 = 0
    parameter  ROI_X16_MIN  = 0,
    parameter  ROI_X16_MAX  = 3,
    parameter  ROI_Y16_MIN  = 0,
    parameter  ROI_Y16_MAX  = 0
) (
    ...
    input  wire        i_pix_tvalid,             // pixel stream valid (4 adjacent pixels)
    output wire        o_pix_tready,             // pixel stream ready (backpressure)
    
    // Per-frame ROI request/response interface (encoder requests ROI once per frame)
    output wire        o_roi_tready,             // asserted by encoder when it needs ROI for the next frame
    input  wire        i_roi_tvalid,             // ROI provider asserts when ROI fields are valid
    input  wire [XL-1:0] i_roi_x16_min,
    input  wire [XL-1:0] i_roi_x16_max,
    input  wire [YL-1:0] i_roi_y16_min,
    input  wire [YL-1:0] i_roi_y16_max,
...
//--------------------------------------------------------------------------------------------------------
// Ready/valid handshakes (pixel input + per-frame ROI fetch)
//--------------------------------------------------------------------------------------------------------

// ROI for current frame (macroblock coordinates). Must be latched BEFORE accepting any pixels of that frame.
reg  [XL-1:0] roi_x16_min_r, roi_x16_max_r;
reg  [YL-1:0] roi_y16_min_r, roi_y16_max_r;
reg           roi_need_r;          // 1 => encoder is requesting ROI for next frame
reg           roi_have_r;          // 1 => ROI received for the current frame

wire roi_fire = o_roi_tready & i_roi_tvalid;   // ROI handshake

// Pixel handshake: accept pixels only when ROI for that frame has been received.
wire pix_fire;

// Encoder requests ROI only when a NEW frame is about to start (before first pixel of that frame).
assign o_roi_tready  = roi_need_r;

// Backpressure to pixel source: ready only after ROI is received, and when sequence isn't ending/ended.
assign o_pix_tready  = roi_have_r && (sequence_state == SEQ_DURING || sequence_state == SEQ_IDLE);

// A pixel "transaction" happens when both valid and ready are asserted.
assign pix_fire = i_pix_tvalid & o_pix_tready;
...
always @ (posedge clk or negedge rstn)
    if (~rstn) begin
        ...
        roi_need_r <= 1'b1;   // need ROI before first frame
        roi_have_r <= 1'b0;
        roi_x16_min_r <= {XL{1'b0}};
        roi_x16_max_r <= {XL{1'b0}};
        roi_y16_min_r <= {YL{1'b0}};
        roi_y16_max_r <= {YL{1'b0}};
    end else begin                                                                                                     //
        sequence_start <= 1'b0;
        // ROI handshake: latch ROI once per frame BEFORE accepting pixels
        if (roi_fire) begin
            roi_x16_min_r <= i_roi_x16_min;
            roi_x16_max_r <= i_roi_x16_max;
            roi_y16_min_r <= i_roi_y16_min;
            roi_y16_max_r <= i_roi_y16_max;
            roi_need_r <= 1'b0;
            roi_have_r <= 1'b1;
        end

        // Detect end-of-frame on accepted pixel of last group
        // (a_x4/max_x4 and a_y/max_y are the running indices in stage A)
        if (pix_fire && sequence_state == SEQ_DURING && a_x4 == max_x4 && a_y == max_y) begin
            // Next frame will need a fresh ROI before accepting pixels
            roi_need_r <= 1'b1;
            roi_have_r <= 1'b0;
        end
        ...
        end else if( pix_fire ) begin                                                                                      // user input a cycle of pixels
...
reg          [  YB16-1:0] g_y16;
reg                       g_inter;
reg                       g_roi;          // ROI flag for this macroblock (1=ROI,0=background)
reg          [       2:0] g_qlevel_mb;    // Q-level actually used for this macroblock
...
always @ (posedge clk)
    if( f_en_blk ) begin
        ...
        g_mvx <= f_mvx;
        g_mvy <= f_mvy;
        // ROI-based Q-level selection using per-frame ROI registers (macroblock coordinates)
        g_roi <= ( (f_x16 >= roi_x16_min_r) && (f_x16 <= roi_x16_max_r) &&
                   (f_y16 >= roi_y16_min_r) && (f_y16 <= roi_y16_max_r) );
        if (g_roi)
            g_qlevel_mb <= Q_LEVEL_ROI;
        else
            g_qlevel_mb <= Q_LEVEL_BG;
        ...
        for (i=0; i<8; i=i+1)
            for (j=0; j<8; j=j+1) begin
                ...
                if( g_inter )                                                                              // inter block
                    g_t3 =   (g_t3 + 16'd2) >> (4 + g_qlevel_mb);                                              //   y = (y+2) / 16 / (1<<Q_LEVEL)
                else if( i!=0 || j!=0 )                                                                    // intra block, AC value
                    g_t3 = ( (g_t3 + ((INTRA_Q[i][j]*((3<<g_qlevel_mb)+2))>>3) ) >> g_qlevel_mb ) / INTRA_Q[i][j]; //   y = ( y + (INTRA_Q*((3<<g_qlevel_mb)+2)>>3) ) / (1<<Q_LEVEL) / INTRA_Q

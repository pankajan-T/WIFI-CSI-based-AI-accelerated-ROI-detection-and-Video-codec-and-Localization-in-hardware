...
module mpeg2encoder_roi #(
    ...
    parameter  Q_LEVEL      = 5,    // quantize level, must be 1, 2, 3 or 4. The larger the Q_LEVEL, the higher compression ratio and the lower quality.
    ...
    parameter  Q_LEVEL_ROI  = 1,   // strong quantization in ROI for visible effect (tune as needed)
    ...
    parameter  ROI_X16_MAX  = 5,
    parameter  ROI_Y16_MIN  = 0,
    parameter  ROI_Y16_MAX  = 5
) (
...
always @ (posedge clk)
    if( f_en_blk ) begin
        ...
        g_mvx <= f_mvx;
        g_mvy <= f_mvy;
        // ROI-based Q-level selection using macroblock coordinates (f_x16,f_y16)
        g_roi <= ( (f_x16 >= ROI_X16_MIN) && (f_x16 <= ROI_X16_MAX) &&
                   (f_y16 >= ROI_Y16_MIN) && (f_y16 <= ROI_Y16_MAX) );
        if (g_roi)
